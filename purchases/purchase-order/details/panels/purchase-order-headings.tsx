...
export default async function PurchaseOrderHeading({ id }: { id: string }) {
  const purchaseOrder: (PurchaseOrderFormData & { id: string }) | undefined =
  ...
  return (
    <div className="d-flex flex-column flex-md-row align-items-md-center justify-content-between mt-2 p-2">
      ...
      <div className="small text-muted">
        {purchaseOrder.vendorName && <span>{purchaseOrder.vendorName} • </span>}
        {purchaseOrder.purchaseOrderNumber && (
          <span>PO#: {purchaseOrder.purchaseOrderNumber}</span>
        )}
        {purchaseOrder.referenceNumber && (
          <>
            {" • "}
            <span>Ref: {purchaseOrder.referenceNumber}</span>
          </>
        )}
      </div>
